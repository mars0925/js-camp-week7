...
function validateOrderUser(data) {
  ...
  if (!data.name || data.name.trim().length === 0) {
    errorsMessages.push("姓名不可為空");
  }

  if (!data.address || data.address.trim().length === 0) {
    errorsMessages.push("地址不可為空");
  }
  if (!telRegex.test(data.tel)) {
    errorsMessages.push("電話必須是 09 開頭的 10 位數字");
  }
  if (!emailRegex.test(data.email)) {
    errorsMessages.push("Email 必須包含 @ 符號");
  }
  if (!validPayments.includes(data.payment)) {
    errorsMessages.push("付款方式必須是 'ATM', 'Credit Card', 'Apple Pay' 其中之一");
  }
  return {
...
1. axios 自動解析 JSON 回應；fetch 需要額外呼叫 .json()

2. axios 在 HTTP 錯誤狀態碼（4xx/5xx）時會拋出例外；fetch 只有在網路錯誤時才拋出

3. axios 會自動設定 Content-Type: application/json；fetch 需手動設定標頭
*/
